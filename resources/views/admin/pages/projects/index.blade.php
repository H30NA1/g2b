        ...
        <div class="row">
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="total-projects ms-3">
                                <h3 class="text-success count">{{ $projects }}</h3>
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <div class="d-flex align-items-center">
                    ...
                    <a href="{{ route('web.projects.create') }}" class="btn btn-primary btn-sm ms-2">+ Add Project</a>
                </div>
            </div>
            @if(!empty($projects))
                    ...
                    <div class="col-xl-3 col-sm-6">
                        <div class="card box-hover">
                            <div class="card-header">
                                <a class="btn btn-danger light ms-1 remove-btn" data-id="{{ $project->id }}">Delete</a>
                                <a class="btn btn-primary me-1" target="_blank" href="{{ route('web.projects.permission',['id' => $project->id]) }}">Permissions</a>
                            </div>
                            <a href="{{ route('web.projects.detail',['id' => $project->id]) }}" target="_blank">
                                <div class="card-body">
                                    <div class="products style-1">
                                        <img src="images/contacts/pic2.jpg" class="avatar avatar-lg rounded-circle" alt="">
                                        <div>
                                            <h6>{{ $project->name }}</h6>
                                            <span>{{ $project->began_at }}</span>
                                        </div>
                                    </div>
                                        ...
                                        <div>
                                            ...
                                            <div class="avatar-list avatar-list-stacked">
                                                @foreach($project->users as $user)
                                                    <img src="{{ $user->user->profile->avatar ?? "" }}" class="avatar rounded-circle" alt="">
                                                @endforeach
                                            </div>
                                        </div>
                                    @endif
                            ...
                            <div class="card-footer d-flex justify-content-between flex-wrap">
                                <div class="due-progress">
                                    <p class="mb-0 text-secondary">Due <span class="text-purple">: {{ $project->deadline_at }}</span></p>
                                </div>
                                <select data-project-id="{{ $project->id }}" class="select-change-status default-select status-select">
...
<script>
    $(document).ready(function() {
        ...
        $('.remove-btn').click(function (e) {
            ...
            let url = @JSON(route('web.projects.delete',['id' => 'changed_id' ]));
            let deleteForm = document.getElementById('deleteForm');
            let actionUrl = url.replace('changed_id',selected_id);
        ...
        $('.select-change-status select').on('change',function (e) {
            ...
            let task_id = parseInt($(this).data('project-id'));
            ...
            let url = @JSON(route('web.projects.change',['id' => 'changed_id']));
