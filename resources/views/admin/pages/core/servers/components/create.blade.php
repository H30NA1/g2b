<form action="{{ route('web.servers.store') }}" method="POST" name="form-submit">
    @csrf
    <div class="row">
    ...
    <div>
        <a class="btn btn-danger light ms-1" href="{{ route('web.servers.index') }}">Back</a>
        <button type="submit" class="btn btn-primary">{{ isset($server) ? 'UPDATE' : 'STORE' }}</button>
    </div>
</form>